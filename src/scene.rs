...
use crate::geometry::{Mat4f, Point3d, Point4d, TraceablePrimitive, PrimitiveType, Vector3d};
...
impl Scene {
    ...
    pub fn add_obj(mut self, obj: SceneObj) -> Self {
		...
		obj.object.to_primitives().into_iter()
			.for_each(|x_model| {
	...
	pub fn accelerate(&mut self) {
		let centroids: Vec<IndexedCentroid> = self.boundboxes.iter()
			.enumerate()
	...
	pub fn build_bvh(&mut self, v: &[IndexedCentroid]) -> usize {
		...
		if min.x == max.x || v.len() < 8 { // TODO refactor
			...
			v.iter().for_each(|&(idx, _)| self.bvh_nodes[node_idx].pointers.push(idx));
	
		} else {
			// we are the inner node
			let mid_value = (min.x + max.x) / 2.0; //TODO refactor
			let (left, right): (Vec<IndexedCentroid>, Vec<IndexedCentroid>) = v.into_iter()
				.partition(|(idx, x)| x.x < mid_value); //TODO add more dimensions
            ...
            if distance_to_bb != None {
                if current_node.children.len() == 0 {
                    ...
                    for &i in &current_node.pointers {
                        let distance_to_obj = self.primitives[i].get_distance_to(ray_orig, ray_dir);
...
impl SceneObj {
	pub(crate) fn new(obj: Arc<dyn IntoPrimitives + Send + Sync>) -> Self {
		SceneObj {
			object: obj,
			scale: [0.0, 0.0, 0.0],
